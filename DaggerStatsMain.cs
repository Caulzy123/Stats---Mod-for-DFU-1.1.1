	...
	public class DaggerStatsMain : MonoBehaviour, IHasModSaveData
	{
	    ...
	    private void InitializeWeaponData(Dictionary<string, int> loadedSavedWeaponData)
        {
            ...
            if (loadedSavedWeaponData != null)
            {
                ...
                //Debug.Log($"[StatsPlus] Weapon data initialized with {weaponData.Count} entries from save.");
            }
            else
            {
                //Debug.Log("[StatsPlus] No saved weapon data, initializing empty weaponData dictionary.");
            }
        }
    	...
    	// 	string regionName;
    	// 	int regionRep;
    	// 	Debug.Log($"[StatsPlus] setup reputation data");

    	// 	foreach (int region in visitedRegions)
    	// 	{
    	// 		Debug.Log($"[StatsPlus] Checking Region # {region}");
    	// 		dfRegion = mapsFile.GetRegion(region);
    	// 		Debug.Log($"[StatsPlus] Region is {dfRegion}");
    	// 		regionName = dfRegion.Name;
    	// 		Debug.Log($"[StatsPlus] Region Name is {regionName}");
    	// 		regionRep = (int)factionData.GetReputation(region);
    	// 		Debug.Log($"[StatsPlus] Region Rep is {regionRep}");

    	// 		Debug.Log($"[StatsPlus] Adding {regionName} with rep of {regionRep} to reputationData");
    	...
    	// 		Debug.Log($"[StatsPlus] Best Reputation is {bestReputationLocation} with {bestReputationValue}");
    	// 		Debug.Log($"[StatsPlus] Worst Reputation is {worstReputationLocation} with {worstReputationValue}");
		...
		public static void CheckLocationTypeData()
		{
            ...
            {
                //Debug.Log($"[StatsPlus] This location has already been discovered.");
                return;
            }
            else
            {
